...
import AddVehicleModal from "../VehicleAddDelete/AddVehicleModal";
...
const VehicleAddDelete = () => {
    ...
    const deleteVehicleResult = useSelector(state => state.deleteVehicle);
    const editVehicleResult = useSelector(state => state.updateVehicle);

    const handleOnClick = async () => {
        dispatch( getAllVehicles());
        console.log('Clicked status', editClicked);
        setEditClicked({
    ...
    const [editClicked, setEditClicked] = useState({ clicked: false, itemSelected: -2});

    const handleEdit = (isClicked, id) => {
        const editClickedObject = {clicked: isClicked, itemSelected: id};
        setEditClicked(editClickedObject);
        console.log('Clicked status handleEdit', editClicked);
    }
            ...
            return data.map(item => {
                ...
                return !editClicked.clicked && (
                    <tr key={item.id}>
                        ...
                        <button className="button2  mr-3" onClick={() => handleEdit(true, id)}>Edit</button>
                        <button className="button3  mr-3" onClick={() => dispatch(deleteVehicle(item.id))}>Delete
                        </button>
                    </tr>
                    ...
                    <tr key={item.id}>
                        ...
                        <td>
                            ...
                            <input value={vehicleFuelType} onChange={(e) => setVehicleFuelType(e.target.value)}/>
                        </td>    <td>
                            <input value={vehicleStatus ? 'true' : 'false'} onChange={(e) => setVehicleStatus(e.target.value)}/>
                        </td>
                        <button className="button2  mr-3" onClick={() => {
                            handleEdit(false, id);
                            dispatch(editVehicleById({vehicleName, vehicleModel, vehicleStatus, vehicleFuelType}))
                        }}>Save</button>
                    </tr>
                );
            });
    ...
    return (
        <div>
            ...
            <div className="container">
                <button className="btn primary mr-3" onClick={handleOnClick} > Vehicles </button>
                <button className="button1  mr-3" onClick={() => setVisibleModal(true)} >Create </button>

                {visibleModal && <AddVehicleModal
                    ...
                    saveModal={() => dispatch(createVehicle({vehicleName, vehicleModel, vehicleStatus, vehicleFuelType}))}
